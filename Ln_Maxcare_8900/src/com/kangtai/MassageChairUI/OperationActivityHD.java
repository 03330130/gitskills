...
import com.kangtai.MassageChairUI.OperationActivityHD.TouchListener;
...
public class OperationActivityHD extends FragmentActivity implements OnTouchListener,OnCheckedChangeListener
	...
	//copy
		...
		private boolean mSleep = false,zuliaoshow;
...
//			Log.d("set state", "-------STATE CODE"+" "+i + readBuf.length);
			...
			case 2:
				...
				setHeatState(b);//1020 trs
				break;
			case 3:
				
				setHeatFeet(b);//1020 trs
				break;
			case 4:
					PreHeat(b);
					PreHeatFeet(b);
...
public void onCheckedChanged(CompoundButton buttonView, boolean isChecked) {
	...
	case R.id.ckb_other_thermal:
		setFeetHeat(isChecked);//1020  bei bu jia re
		break;
	case R.id.ckb_other_feet_thermal:
		
		setBackHeat(isChecked);  //1020 jia bu jiare
		break;
...
//			if (touchSoundFlag) {
	...
	if(buttonView.getId()==R.id.ckb_ksty)
	{
		if(kstyFlag){  
			...
			zuliaoOn=false;ckb_zuliao.setChecked(false);//guan bi zuliao xianshi
